...
\section{Úvod}
V dobe, kedy digitálne údaje sú veľmi dôležité nielen pre firmy, ale aj pre jednotlivcov, je nevyhnutné mať spoľahlivé riešenie na ukladanie a zálohovanie údajov. Tento projekt sa zameriava na návrh spoľahlivého systému ukladania a zálohovania údajov, ktorý môže bežná osoba prevádzkovať v pohodlí svojho domova. Pôjde o vytvorenie systému NAS\footnote{Network Attached Storage} využívajúci bežne dostupné počítačové komponenty a sieťovú infraštruktúru. Projekt sa venuje aj výberu vhodného softvéru, ktorý efektívne využije možnosti systému NAS. Okrem toho sa zaoberá umiestnením NAS v domácom prostredí, aby bolo zabezpečené pohodlné používanie bez zbytočného rušenia. Dôležitou súčasťou projektu je aj porovnanie nákladov medzi domácou a komerčnou alternatívou.
...
\end{tabularx}
\caption{Parametre základnej dosky ASUS PRIME B450M-A II}
\end{table}
...
L3 cache & 8 MB \\ \hline
\end{tabularx}
\caption{Parametre procesora AMD Ryzen 5 4600G}
\end{table}
...
\end{tabularx}
\captionsetup{justification=centering,margin=2cm}
\caption{Parametre pamäte Kingston FURY 16 GB KIT DDR4 3200 MHz CL16 Beast Black}
\end{table}
...
\end{tabularx}
\caption{Parametre SSD disku Patriot P300 128GB}
\end{table}
...
\end{tabularx}
\caption{Parametre dátového disku Seagate IronWolf Pro 2TB}
\end{table}
...
\end{tabularx}
\caption{Parametre zdroja Be quiet! PURE POWER 11 400 W}
\end{table}
...
Farba & Sivá \\ \hline
\end{tabular}
\caption{Parametre sieťového kábla}
\end{table}
...
Farba & Žltá \\ \hline
\end{tabular}
\caption{Parametre SATA kábla}
\end{table}



\section{Softvér}
Zo softvérovej časti je treba si zvoliť aký typ redundancie dát chceme a aký operačný systém použijeme pre náš projekt.
...
\begin{spacing}{1.0}
    \begin{itemize}
        ...
        \item RAID 5
            \begin{itemize}
                \item Používa jeden disk na paritné dáta. Dáta a parita sú rozdelené medzi tri alebo viac diskov. Poskytuje redundanciu pri zlyhaní jedného disku.
            \end{itemize}
        \item RAID 6
            \begin{itemize}
...
Ako prvú máme tabuľku s cenami spoločných položiek pre všetky riešenia. Sú tam zahrnuté dátové disky a sieťová kábel z dôvodu, že pri všetkých riešeniach by sme tieto položky potrebovali.
...
\end{tabularx}
\caption{Parametre pre Synology DS923+}
\end{table}
